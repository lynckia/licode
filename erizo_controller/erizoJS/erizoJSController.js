...
exports.ErizoJSController = function (erizoAgentID, erizoJSID, threadPool, ioThreadPool) {
    ...
    enablePublisherTimeout = function(wrtc, idPub, idSub) {
        if (!global.config.erizoAgent.publisherTimeout) {
            return;
        }
        if (idSub !== undefined) {
            return;
            ...
            wrtc.getStats((statsString) => {
                ...
                if (!somethingHappened) {
                    ...
                    if (publisher.keepAliveCounter >=
                        global.config.erizoAgent.publierTimeout.keepAliveCount) {
                        that.removePublisher(idPub);
                    }
                } else {
                    publisher.keepAliveCounter = 0;
        ...
        }, global.config.erizoAgent.publisherTimeout.intervalMs);
    };
