class RovMetricsGatherer {
  ...
  getTotalRooms() {
    return new Promise((resolve, reject) => {
      ...
      this.rovClient.runInComponentList('console.log(context.rooms.size)', this.rovClient.components.erizoControllers)
        .then((results) => {
          let totalRooms = 0;
        ...
        .catch((error) => {
          this.log.error('Error in getTotalRooms', error);
          reject(error);
        });
    });
  }
  ...
  getTotalClients() {
    return new Promise((resolve, reject) => {
      ...
      this.rovClient.runInComponentList('var totalClients = 0; context.rooms.forEach((room) => {totalClients += room.clients.size}); console.log(totalClients);',
        ...
        .catch((error) => {
          this.log.error('Error in getTotalClients', error);
          reject(error);
        });
    });
  }
  ...
  getTotalPublishersAndSubscribers() {
    return new Promise((resolve, reject) => {
      ...
      Promise.all(requestPromises).then((results) => {
        ...
        .catch((error) => {
          this.log.error('Error in getTotalPublishers', error);
          reject(error);
        });
    });
  }
