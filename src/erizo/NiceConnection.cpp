...
void NiceConnection::init(){
	...
	// Connect the signals
	g_signal_connect( G_OBJECT( agent_ ), "candidate-gathering-done", G_CALLBACK( cb_candidate_gathering_done ), this );
	g_signal_connect( G_OBJECT( agent_ ), "component-state-changed", G_CALLBACK( cb_component_state_changed ), NULL );
	g_signal_connect( G_OBJECT( agent_ ), "new-selected-pair", G_CALLBACK( cb_new_selected_pair ), this );

	// Create a new stream and start gathering candidates

	int res = nice_agent_add_stream( agent_, 2);
	...
	nice_agent_attach_recv( agent_, 1, 1, g_main_loop_get_context( loop_ ), cb_nice_recv, this );
...
bool NiceConnection::setRemoteCandidates(std::vector<CandidateInfo> &candidates){
	...
	for (unsigned int it =0; it<candidates.size(); it++){
		...
		if (cinfo.mediaType != this->mediaType||this->transportName->compare(cinfo.transProtocol))
			continue;
