...
bool WebRtcConnection::setRemoteSdp(const std::string &sdp){
	...
	for (unsigned int it = 0; it < crypto_remote.size(); it++){
		...
		if (cryptemp.mediaType == VIDEO_TYPE && !cryptemp.cipherSuite.compare("AES_CM_128_HMAC_SHA1_80")){
			video_ = true;
			cryptRemote_video = cryptemp;
		}else if (cryptemp.mediaType == AUDIO_TYPE && !cryptemp.cipherSuite.compare("AES_CM_128_HMAC_SHA1_80")){
			audio_ = true;
	...
	if (!bundle_){
		if(video_){
			...
			videoSrtp_->setRtpParams((char*)cryptLocal_video.keyParams.c_str(), (char*)cryptRemote_video.keyParams.c_str());
