...
install_apt_deps(){
  ...
  npm install gulp@3.9.1 gulp-eslint@3 run-sequence@2.2.1 webpack-stream@4.0.0 google-closure-compiler-js@20170521.0.0 \
	del@3.0.0 gulp-sourcemaps@2.6.4 script-loader@0.7.2 expose-loader@0.7.5
  ...
  sudo apt-get install -qq automake autoconf libssl-dev cmake libglib2.0-dev pkg-config libboost-regex-dev \
	libboost-thread-dev libboost-system-dev liblog4cxx10-dev rabbitmq-server mongodb curl libboost-test-dev -y
...
install_mediadeps(){
  ...
  if [ -d $LIB_DIR ]; then
    ...
    if [ ! -f ./v11.9.tar.gz ]; then
      ...
      cd libav-11.9
      PKG_CONFIG_PATH=${PREFIX_DIR}/lib/pkgconfig ./configure --prefix=$PREFIX_DIR --enable-shared \
	--enable-gpl --enable-libvpx --enable-libx264 --enable-libopus --disable-doc
      make $FAST_MAKE -s V=0
      make install
    else
...
install_mediadeps_nogpl(){
  ...
  if [ -d $LIB_DIR ]; then
    ...
    if [ ! -f ./v11.9.tar.gz ]; then
      ...
      cd libav-11.9
      PKG_CONFIG_PATH=${PREFIX_DIR}/lib/pkgconfig ./configure --prefix=$PREFIX_DIR --enable-shared \
	--enable-gpl --enable-libvpx --enable-libopus --disable-doc
      make $FAST_MAKE -s V=0
      make install
    else
      echo "libav already installed"
