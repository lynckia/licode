...
void TransportFeedbackAdapter::AddPacket(const RtpPacketSendInfo& packet_info,
  ...
  // printf("Inserting packet %u, unwrapped %u \n",
...
absl::optional<SentPacket> TransportFeedbackAdapter::ProcessSentPacket(
  ...
  if (sent_packet.info.included_in_feedback || sent_packet.packet_id != -1) {
    ...
    // printf("Sent packet is being processed %u, unwrapped%d\n", sent_packet.packet_id,
    // unwrapped_seq_num);
    auto it = history_.find(unwrapped_seq_num);
    if (it != history_.end()) {
      // printf("Packet found in history %d\n", unwrapped_seq_num);
      ...
      if (!packet_retransmit) {
        // printf("It's not retransmit %u, last_ack %u\n", it->second.sent.sequence_number,
        // last_ack_seq_num_);
...
TransportFeedbackAdapter::ProcessTransportFeedback(
  ...
  // printf("data_in_flight %u\n", msg.data_in_flight.bytes());
...
TransportFeedbackAdapter::ProcessTransportFeedbackInner(
  ...
  for (const auto& packet : feedback.GetAllPackets()) {
    ...
    if (seq_num > last_ack_seq_num_) {
      ...
      // printf("We have an ack in the feedback %u\n", last_ack_seq_num_);
  ...
  // printf("Packet status count %u, ignored %u, failed %u\n", feedback.GetPacketStatusCount(), failed_lookups, ignored);
