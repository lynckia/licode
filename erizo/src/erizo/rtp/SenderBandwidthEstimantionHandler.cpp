...
SenderBandwidthEstimationHandler::SenderBandwidthEstimationHandler() :
  connection_{nullptr}, bwe_listener_{nullptr}, initialized_{false}, enabled_{true}, received_remb_{false},
  period_packets_sent_{0}, estimated_bitrate_{0}, estimated_loss_{0}, estimated_rtt_{0},
  last_estimate_update_{clock::now()}, sender_bwe_{new SendSideBandwidthEstimation()} {
        ...
        case RTCP_Receiver_PT:
          {
            ...
            if (received_remb_ && value != sr_delay_data_.end()) {
                ...
                ELOG_DEBUG("%s message: Updating Estimate with RR, fraction_lost: %u, "
                    "delay: %u, period_packets_sent_: %u, now_ms: %u",
                    connection_->toLog(), chead->getFractionLost(), delay, period_packets_sent_, now_ms);
...
void SenderBandwidthEstimationHandler::write(Context *ctx, std::shared_ptr<dataPacket> packet) {
  ...
  if (!chead->isRtcp() && packet->type == VIDEO_PACKET) {
    ...
    if (now - last_estimate_update_ > kMinUpdateEstimateInterval) {
