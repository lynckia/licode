...
void SenderBandwidthEstimationHandler::read(Context *ctx, std::shared_ptr<DataPacket> packet) {
  ...
  if (chead->isFeedback()) {
    ...
    do {
      ...
      ELOG_DEBUG("%s ssrc %u, sourceSSRC %u, PacketType %u", log_str.c_str(),
          chead->getSSRC(),
          chead->getSourceSSRC(),
          chead->getPacketType());
        ...
        case RTCP_Receiver_PT:
          {
            ...
            if (received_remb_ && value != sr_delay_data_.end()) {
                ...
                ELOG_DEBUG("%s message: Updating Estimate with RR, fraction_lost: %u, "
                    ...
                    log_str.c_str(), chead->getFractionLost(), delay, period_packets_sent_);
        ...
        case RTCP_PS_Feedback_PT:
          {
            if (chead->getBlockCount() == RTCP_AFB) {
              ...
              if (!strncmp(uniqueId, "REMB", 4)) {
                ...
                const auto log_str = stream_->toLog();
                ELOG_DEBUG("%s message: Updating Estimate with REMB, bitrate %lu", log_str.c_str(),
                    cappedBitrate);
                sender_bwe_->UpdateReceiverEstimate(now_ms, cappedBitrate);
                updateEstimate();
              } else {
                const auto log_str = stream_->toLog();
                ELOG_DEBUG("%s message: Unsupported AFB Packet not REMB", log_str.c_str());
              }
            }
          }
          break;
...
void SenderBandwidthEstimationHandler::analyzeSr(RtcpHeader* chead) {
  ...
  const auto log_str = stream_->toLog();
  ELOG_DEBUG("%s message: adding incoming SR to list, ntp: %u", log_str.c_str(), ntp);
...
void SenderBandwidthEstimationHandler::updateEstimate() {
  ...
  ELOG_DEBUG("%s message: estimated bitrate %d, loss %u, rtt %ld",
      log_str.c_str(), estimated_bitrate_, estimated_loss_, estimated_rtt_);
  if (bwe_listener_) {
